...
argument_list|()
expr_stmt|;
name|loadChangeScreen
argument_list|()
expr_stmt|;
block|}
DECL|method|loadChangeScreen ()
specifier|private
name|void
name|loadChangeScreen
parameter_list|()
...
condition|)
block|{
comment|// Load the project if it is not already present. This is the case when the user used a URL
comment|// that doesn't include the project. Setting it here will rewrite the URL token to include the
comment|// project (visible to the user) and all future API calls made from the change screen will use
...
block|{
comment|// Initialize current screen with newly obtained project
name|project
operator|=
name|result
operator|.
