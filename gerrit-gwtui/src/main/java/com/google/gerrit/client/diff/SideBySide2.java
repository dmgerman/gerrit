...
argument_list|)
expr_stmt|;
comment|// TODO: Prevent right click from updating the cursor.
name|cm
operator|.
name|addKeyMap
argument_list|(
...
block|}
block|}
DECL|method|getCommentCallback ( final boolean toDrafts)
specifier|private
name|GerritCallback
argument_list|<
name|NativeMap
...
return|;
block|}
DECL|method|addAllToCommentList (JsArray<CommentInfo> in, boolean toDrafts)
specifier|private
name|void
name|addAllToCommentList
parameter_list|(
...
block|}
block|}
DECL|method|addDraftBox (CommentInfo info)
name|DraftBox
name|addDraftBox
parameter_list|(
name|CommentInfo
...
return|;
block|}
DECL|method|removeDraft (DraftBox box, Side side, int line)
name|void
name|removeDraft
parameter_list|(
name|DraftBox
...
expr_stmt|;
block|}
DECL|method|removeFileCommentBox (DraftBox box, Side side)
name|void
...
return|;
block|}
DECL|method|getPatchSetIdFromSide (Side side)
specifier|private
name|PatchSet
operator|.
name|Id
...
block|}
block|}
DECL|method|addDiffChunkAndPadding (CodeMirror cmToPad, int lineToPad, int lineOnOther, int chunkSize)
specifier|private
name|void
name|addDiffChunkAndPadding
parameter_list|(
...
parameter_list|()
block|{
comment|// Hack to prevent feedback loop, Chrome seems fine but Firefox chokes.
if|if
condition|(
name|cm
operator|.
...
expr_stmt|;
block|}
comment|/**          * Since CM doesn't always take the height of line widgets into          * account when calculating scrollInfo when scrolling too fast          * (e.g. throw-scrolling), simply setting scrollTop to be the same          * doesn't guarantee alignment, but should work in most cases. See the          * hack in fixScroll();          */
name|other
operator|.
name|scrollToY
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/**      * If the line in the middle of the viewPort isn't part of an insertion /      * deletion gap, isAligned() will be true, and hopefully this will be      * the majority of cases. We then manually examine if the lines that      * should be aligned are at the same height. If not, perform additional      * scrolling until the lines are aligned.      */
if|if
condition|(
name|info
operator|.
...
return|;
block|}
DECL|method|getDiffChunk (Side side, int line)
specifier|private
name|DiffChunkInfo
name|getDiffChunk
parameter_list|(
...
return|;
block|}
DECL|method|resizePaddingOnOtherSide (Side mySide, int line)
name|void
...
DECL|method|resizeLinePadding (final Side side)
specifier|private
name|RenderLineHandler
name|resizeLinePadding
parameter_list|(
...
name|end
decl_stmt|;
DECL|method|DiffChunkInfo (Side side, int start, int end)
name|DiffChunkInfo
parameter_list|(
name|Side
name|side
