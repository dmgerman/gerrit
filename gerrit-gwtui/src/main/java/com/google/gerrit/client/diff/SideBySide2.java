...
decl_stmt|;
DECL|field|linePaddingWidgetMap
specifier|private
name|Map
argument_list|<
name|LineHandle
...
name|LinePaddingWidgetWrapper
argument_list|>
name|linePaddingWidgetMap
decl_stmt|;
DECL|field|lineElementMap
specifier|private
name|Map
argument_list|<
name|LineHandle
...
argument_list|)
expr_stmt|;
comment|/**      * TODO: Work on a better way for customizing keybindings and remove      * temporary navigation hacks.      */
for|for
control|(
name|String
name|s
...
block|}
block|}
DECL|method|insertEmptyLines (CodeMirror cm, int nextLine, int lineOnOther, int cnt, boolean common)
specifier|private
name|void
name|insertEmptyLines
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|/**        * Link a line with its padding widget on the other side. Note that we        * also add a collapsed padding for common lines, because they may        * linewrap differently due to intraline edits, in which case we detect        * the difference in line height and resize the padding widget.        */
name|linePaddingWidgetMap
operator|.
name|put
argument_list|(
...
condition|)
block|{
comment|/**            * This needs to be deferred because CM fires "renderLine" **before**            * the line is actually added to the DOM.            */
name|Scheduler
operator|.
name|get
argument_list|()
...
block|{
comment|/**                  * We have to pay the cost of keeping track of the actual DOM                  * elements ourselves, because CM doesn't provide an interface                  * to query for them, and the only place we can ever have legit                  * access to the line element is in this event handler.                  */
name|lineElementMap
operator|.
name|put
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// The lines are always aligned since they are in a common region.
name|int
name|otherLine
init|=
name|mapper
