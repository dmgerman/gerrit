...
expr_stmt|;
block|}
comment|// Else ignore the result, the user has already changed subname and
comment|// the result is not relevant anymore. If multiple RPC's are fired
comment|// the results may come back out-of-order and a non-relevant result
comment|// could overwrite the correct result if not ignored.
block|}
block|}
argument_list|)
expr_stmt|;
block|}
DECL|method|display (final ProjectMap result)
specifier|private
name|void
name|display
parameter_list|(
