...
annotation|@
name|Inject
DECL|method|AccountManager (final SchemaFactory<ReviewDb> schema, final AccountCache byIdCache, final AccountByEmailCache byEmailCache, final EmailExpander emailExpander, final AuthConfig authConfig)
name|AccountManager
parameter_list|(
specifier|final
name|SchemaFactory
...
name|append
argument_list|(
literal|"Multiple accounts match \""
argument_list|)
expr_stmt|;
name|r
operator|.
...
condition|)
block|{
comment|// A SCHEMA_GERRIT account was authenticated by an external SSO
comment|// solution. The external identity string actually contains a
comment|// name that we can uniquely refer to the user by, so set
comment|// account information based upon that name.
comment|//
specifier|final
name|String
name|user
