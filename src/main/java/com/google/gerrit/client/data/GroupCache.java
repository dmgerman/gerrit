...
block|}
block|}
comment|/**    * Get the groups a specific account is a member of.    *<p>    * A user is only a member of groups beyond {@link #anonymousGroupId} and    * {@link #registeredGroupId} if their account is using only    * {@link TrustedExternalId}s.    *     * @param accountId the account to obtain the group list for.    * @return unmodifiable set listing the groups the account is a member of.    */
DECL|method|getEffectiveGroups (final Account.Id accountId)
specifier|public
name|Set
argument_list|<
name|AccountGroup
...
return|;
block|}
DECL|method|isIdentityTrustable (final ReviewDb db, final Account.Id accountId)
specifier|private
name|boolean
name|isIdentityTrustable
parameter_list|(
...
case|:
default|default:
comment|// Validate against the trusted provider list
comment|//
return|return
name|TrustedExternalId
operator|.
