...
return|;
block|}
comment|/**    * Reload the zip {@link FileSystem} for a path.    *<p>    *<strong>Warning</strong>: This calls {@link FileSystem#close()} on any    * previously open instance of the filesystem at this path, which may cause    * {@code IOException}s in any open path handles created with the old    * filesystem. Use with caution.    *    * @param zip path to zip file.    * @return reloaded filesystem instance.    * @throws IOException if there was an error reading the zip file.    */
DECL|method|reloadZipFileSystem (Path zip)
specifier|public
specifier|static
specifier|synchronized
name|FileSystem
...
literal|"resource"
argument_list|)
comment|// Caching resource for later use.
name|FileSystem
name|zipFs
init|=
name|zipFileSystems
