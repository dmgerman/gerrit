...
block|{
comment|/**    * Construct a token to verify an email address for a user.    *    * @param accountId the caller that wants to add an email to their account.    * @param emailAddress the address to add.    * @return an unforgeable string to email to {@code emailAddress}. Presenting    *         the string provides proof the user has the ability to read messages    *         sent to that address.    */
...
parameter_list|)
function_decl|;
comment|/**    * Decode a token previously created.    * @param tokenString the string created by encode.    * @return a pair of account id and email address.    * @throws InvalidTokenException the token is invalid, expired, malformed, etc.    */
