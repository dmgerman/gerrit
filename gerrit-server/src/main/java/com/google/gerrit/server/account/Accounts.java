...
annotation|@
name|Inject
DECL|method|Accounts ( @erritServerConfig Config cfg, GitRepositoryManager repoManager, AllUsersName allUsersName, OutgoingEmailValidator emailValidator, ExternalIds externalIds)
name|Accounts
parameter_list|(
annotation|@
name|GerritServerConfig
...
return|;
block|}
DECL|method|get (ReviewDb db, Collection<Account.Id> accountIds)
specifier|public
name|List
argument_list|<
name|Account
...
return|;
block|}
comment|/**    * Returns the accounts for the given emails.    *    *<p>Each email should belong to a single account only. This means if more than one account for    * an email is returned there is an inconsistency in the external IDs.    *    *<p>The accounts are retrieved via the external ID cache. Each access to the external ID cache    * requires reading the SHA1 of the refs/meta/external-ids branch. If accounts for multiple emails    * are needed it is more efficient to use this method instead of {@link #byEmail(String)} as this    * method reads the SHA1 of the refs/meta/external-ids branch only once (and not once per email).    *    * @see #byEmail(String)    */
