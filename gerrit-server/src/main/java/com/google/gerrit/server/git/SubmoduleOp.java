...
argument_list|()
decl_stmt|;
comment|// If SSH is disabled on the command line there will be no
comment|// sshProvider to ask. Translate this to the gerrit.config
comment|// option "off" inside the [sshd] section.
if|if
condition|(
name|sshProvider
operator|==
...
block|{
comment|// Make sure we only remove the port part and not a substring
comment|// of an IPv6 address.
name|sshHostname
operator|=
name|sshProvider
operator|.
...
expr_stmt|;
block|}
comment|// Use the web host name (canonical web url) for the following
comment|// conditions:
comment|// - If the ssh hostname is the same as the web host
comment|// - If we listen on all addresses
comment|// - If sshd is turned off
if|if
condition|(
name|webHostname
operator|.
