...
comment|//
comment|// It's not possible to share a single inserter for All-Users across all project
comment|// tasks, and we don't want to add one pack per project to All-Users. Adding many
comment|// loose objects is preferable to many packs.
comment|//
comment|// Anyway, the number of objects inserted into All-Users is proportional to the
comment|// number of pending draft comments, which should not be high (relative to the total
comment|// number of changes), so the number of loose objects shouldn't be too unreasonable.
operator|.
name|setChangeRepo
argument_list|(
name|changeRepo
...
begin_function
DECL|method|stage (ReviewDb db, Change.Id changeId, NoteDbUpdateManager manager)
specifier|private
name|void
name|stage
parameter_list|(
