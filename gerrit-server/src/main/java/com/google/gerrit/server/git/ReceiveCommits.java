...
comment|// TODO(dborowitz): Corner case where an existing commit might need a
comment|// new group:
comment|// Let A<-B<-C, then:
...
comment|// B will be in existing so we aren't replacing the patch set. It used
comment|// to have its own group, but now needs to to be changed to A's group.
continue|continue;
block|}
if|if
condition|(
...
return|;
block|}
DECL|method|newChangeMessage (ReviewDb db, ChangeKind changeKind)
specifier|private
name|ChangeMessage
