...
argument_list|()
decl_stmt|;
comment|// The range field for a comment can be null. If a comment's range is null
comment|// we just use the line field in the comment (which can't be null) for
comment|// comparison and compare to the other comment's range's end line (if
comment|// its range isn't null) or just the line field in the other comment.
name|int
name|lineForComment1
init|=
operator|(
name|range1
operator|==
...
expr_stmt|;
block|}
DECL|method|parseComments (RevCommit commit)
specifier|private
name|void
name|parseComments
parameter_list|(
