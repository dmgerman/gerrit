...
block|}
block|}
DECL|method|recheckUpToDate (Repository repo, IOException e)
specifier|private
name|NoteDbChangeState
name|recheckUpToDate
parameter_list|(
...
name|IOException
block|{
comment|// Should only be non-null if auto-rebuilding disabled.
name|checkState
argument_list|(
name|refs
operator|==
...
argument_list|)
expr_stmt|;
comment|// An error during auto-rebuilding might be caused by LOCK_FAILURE or a
comment|// similar contention issue, where another thread successfully rebuilt the
comment|// change. Reread the change from ReviewDb and NoteDb and recheck the state.
name|Change
name|newChange
decl_stmt|;
try|try
...
return|;
block|}
DECL|method|logRecheckError (Throwable t)
specifier|private
name|void
name|logRecheckError
parameter_list|(
...
name|error
argument_list|(
literal|"Error rechecking if change "
operator|+
name|getChangeId
argument_list|()
operator|+
literal|" is up to date; logging this exception but rethrowing original"
argument_list|,
name|t
argument_list|)
expr_stmt|;
