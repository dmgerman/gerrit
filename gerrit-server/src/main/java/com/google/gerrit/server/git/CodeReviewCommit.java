...
return|;
block|}
comment|/**    * Create an error commit.    *<p>    * Should only be used for error statuses such that there is no possible    * non-zero commit on which we could call {@link    * #setStatusCode(CommitMergeStatus)}, enumerated in the methods above.    *    * @param CommitMergeStatus status    * @return new commit instance    */
DECL|method|error (CommitMergeStatus s)
specifier|private
specifier|static
name|CodeReviewCommit
name|error
