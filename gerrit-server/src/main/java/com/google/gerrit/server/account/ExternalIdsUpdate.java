...
name|retryer
decl_stmt|;
DECL|method|ExternalIdsUpdate ( GitRepositoryManager repoManager, AccountCache accountCache, AllUsersName allUsersName, PersonIdent committerIdent, PersonIdent authorIdent)
specifier|private
name|ExternalIdsUpdate
parameter_list|(
name|GitRepositoryManager
...
annotation|@
name|VisibleForTesting
DECL|method|ExternalIdsUpdate ( GitRepositoryManager repoManager, AccountCache accountCache, AllUsersName allUsersName, PersonIdent committerIdent, PersonIdent authorIdent, Runnable afterReadRevision, Retryer<Void> retryer)
specifier|public
name|ExternalIdsUpdate
parameter_list|(
name|GitRepositoryManager
...
expr_stmt|;
block|}
comment|/**    * Removes an external ID from the note map by external ID key.    *    *<p>The external ID is only deleted if it belongs to the specified account. If the external IDs    * belongs to another account the deletion fails with {@link IllegalStateException}.    */
DECL|method|remove ( RevWalk rw, NoteMap noteMap, Account.Id accountId, ExternalId.Key extIdKey)
specifier|private
specifier|static
name|void
name|remove
...
name|update
decl_stmt|;
DECL|method|TryNoteMapUpdate ( Repository repo, RevWalk rw, ObjectInserter ins, MyConsumer<OpenRepo> update)
specifier|private
name|TryNoteMapUpdate
parameter_list|(
name|Repository
