...
annotation|@
name|Inject
DECL|method|FillArgs (Provider<ReviewDb> db, GitRepositoryManager repoManager, PatchListCache patchListCache, ChangeControl.GenericFactory changeControlFactory, InternalUser internalUser)
name|FillArgs
parameter_list|(
name|Provider
argument_list|<
...
expr_stmt|;
block|}
comment|/**      * Create a change control.      *      * @param change change object.      * @return change control as seen by Gerrit's internal user; index fields      *     may not depend on any per-user state.      */
DECL|method|changeControlFor (Change change)
name|ChangeControl
name|changeControlFor
parameter_list|(
name|Change
name|change
parameter_list|)
