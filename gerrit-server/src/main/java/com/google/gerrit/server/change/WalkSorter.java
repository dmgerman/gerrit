...
argument_list|)
expr_stmt|;
comment|// Mark parents of all merge bases as uninteresting, to limit the cost of
comment|// the topo sort, which slurps all interesting commits before returning
comment|// anything.
comment|//
comment|// There are two cases where this doesn't work and performance will
comment|// suffer:
comment|// 1. The common ancestor is a root commit; JGit currently does not return
comment|//    a root commit as a merge base, even if it is the common ancestor.
comment|// 2. There is a merge of two unrelated branches of history; these simply
comment|//    have no merge base, so this technique of finding uninteresting
comment|//    ancestors doesn't work.
comment|//
comment|// The performance impact of these cases in practice is hopefully low;
comment|// this does handle the common case of a single chain of commits.
for|for
control|(
name|RevCommit
name|mergeBase
...
block|}
block|}
DECL|method|findMergeBases (RevWalk rw, Iterable<RevCommit> commits)
specifier|private
specifier|static
name|List
argument_list|<
