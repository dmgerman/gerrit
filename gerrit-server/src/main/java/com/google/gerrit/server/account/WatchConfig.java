...
begin_comment
comment|/**  * âwatch.configâ file in the user branch in the All-Users repository that  * contains the watch configuration of the user.  *<p>  * The 'watch.config' file is a git config file that has one 'project' section  * for all project watches of a project.  *<p>  * The project name is used as subsection name and the filters with the notify  * types that decide for which events email notifications should be sent are  * represented as 'notify' values in the subsection. A 'notify' value is  * formatted as '<filter> [<comma-separated-list-of-notify-types>]':  *  *<pre>  *   [project "foo"]  *     notify = * [ALL_COMMENTS]  *     notify = branch:master [ALL_COMMENTS, NEW_PATCHSETS]  *     notify = branch:master owner:self [SUBMITTED_CHANGES]  *</pre>  *<p>  * If two notify values in the same subsection have the same filter they are  * merged on the next save, taking the union of the notify types.  *<p>  * For watch configurations that notify on no event the list of notify types is  * empty:  *  *<pre>  *   [project "foo"]  *     notify = branch:master []  *</pre>  *<p>  * Unknown notify types are ignored and removed on save.  */
end_comment
...
annotation|@
name|VisibleForTesting
DECL|method|parse ( Account.Id accountId, Config cfg)
specifier|public
specifier|static
name|Map
argument_list|<
...
name|NotifyValue
block|{
DECL|method|parse (Account.Id accountId, String project, String notifyValue)
specifier|public
specifier|static
name|NotifyValue
name|parse
