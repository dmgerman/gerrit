...
annotation|@
name|Inject
DECL|method|PublishDraftPatchSet ( Provider<ReviewDb> dbProvider, BatchUpdate.Factory updateFactory, ChangeHooks hooks, ApprovalsUtil approvalsUtil, AccountResolver accountResolver, PatchSetInfoFactory patchSetInfoFactory, CreateChangeSender.Factory createChangeSenderFactory, ReplacePatchSetSender.Factory replacePatchSetFactory)
specifier|public
name|PublishDraftPatchSet
parameter_list|(
name|Provider
...
condition|)
block|{
comment|// Don't save in patchSet, since we're not in a transaction. Here we
comment|// just need the revision, which is immutable.
name|ps
operator|=
name|ctx
operator|.
