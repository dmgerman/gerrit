...
name|HttpServlet
block|{
DECL|field|SIGNIN_MODE_PARAMETER
specifier|private
specifier|static
specifier|final
name|String
name|SIGNIN_MODE_PARAMETER
init|=
name|SignInDialog
operator|.
name|SIGNIN_MODE_PARAM
decl_stmt|;
...
expr_stmt|;
block|}
DECL|method|chooseProvider (final HttpServletRequest req, final HttpServletResponse rsp)
specifier|private
name|void
name|chooseProvider
parameter_list|(
...
condition|)
block|{
comment|// Try to make the entry URL match what we expect it to be, in
comment|// case the OpenID provider relies on this to generate tokens
comment|// (some do, like Google Accounts).
comment|//
name|rsp
operator|.
name|setStatus
...
return|return;
block|}
comment|// TODO Should be in init so we can cache.
specifier|final
name|Document
name|loginDoc
decl_stmt|;
...
name|loginPageName
init|=
literal|"com/google/gerrit/public/Login.html"
decl_stmt|;
name|loginDoc
operator|=
name|HtmlDomUtil
...
name|write
argument_list|(
literal|"var gerrit_openid_identifier="
argument_list|)
expr_stmt|;
specifier|final
name|String
...
block|}
block|}
DECL|method|openAccount (final ReviewDb db, final OpenIdUser user, final String email)
specifier|private
name|Account
name|openAccount
parameter_list|(
