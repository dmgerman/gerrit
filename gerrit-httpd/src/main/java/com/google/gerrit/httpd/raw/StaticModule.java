...
argument_list|)
expr_stmt|;
comment|// TODO(dborowitz): These fragments conflict with the REST API namespace, so
comment|// they will need to use a different path.
comment|//serve("/groups/*").with(PolyGerritUiIndexServlet.class);
comment|//serve("/projects/*").with(PolyGerritUiIndexServlet.class);
if|if
condition|(
name|warFs
operator|==
...
name|BOWER_SERVLET
argument_list|)
DECL|method|getPolyGerritUiBowerServlet ( @amedCACHE) Cache<Path, Resource> cache)
name|PolyGerritUiServlet
name|getPolyGerritUiBowerServlet
parameter_list|(
annotation|@
name|Named
argument_list|(
