...
name|ctx
init|=
name|createBlockReviewDbGroupUpdatesContext
argument_list|()
init|)
block|{
name|gApi
...
try|try
block|{
comment|// On startup of the slave the test framework ensures that the group index is up-to-date.
comment|// Hence running the reindexer doesn't need to reindex any group.
name|slaveGroupIndexer
operator|.
name|run
argument_list|()
...
return|;
block|}
DECL|method|createBlockReviewDbGroupUpdatesContext ()
specifier|private
name|AutoCloseable
name|createBlockReviewDbGroupUpdatesContext
parameter_list|()
