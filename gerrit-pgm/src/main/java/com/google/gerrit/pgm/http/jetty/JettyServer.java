...
annotation|@
name|Inject
DECL|method|JettyServer (@erritServerConfig final Config cfg, final SitePaths site, final JettyEnv env, final Provider<CurrentUser> userProvider)
name|JettyServer
parameter_list|(
annotation|@
name|GerritServerConfig
...
name|ServletException
block|{
comment|// Force the user to construct, so it's available to our HttpLog
comment|// later on when the request gets logged out to the access file.
comment|//
name|GuiceHelper
operator|.
name|runInContext
...
expr_stmt|;
block|}
comment|// We should be in a Maven style output, that is $jar/target/classes.
comment|//
if|if
condition|(
operator|!
