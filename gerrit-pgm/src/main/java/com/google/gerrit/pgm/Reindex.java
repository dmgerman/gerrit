...
return|;
block|}
comment|/**    * Subset of GerritGlobalModule including just bindings needed for indexing.    *    * Does<b>not</b> include thread local request scoping behavior of ReviewDbs;    * uses a custom {@link ReviewDbModule} instead.    */
DECL|class|GlobalModule
specifier|private
class|class
name|GlobalModule
extends|extends
...
expr_stmt|;
block|}
comment|// No need to support live plugin removal or other live updates of
comment|// caches behind our back, so don't worry about cache removal.
name|bind
argument_list|(
operator|new
name|TypeLiteral
...
argument_list|)
expr_stmt|;
comment|// Assume the current user is always the Gerrit internal user; no
comment|// indexed data should depend on what user is doing the indexing.
name|bind
argument_list|(
name|CurrentUser
operator|.
