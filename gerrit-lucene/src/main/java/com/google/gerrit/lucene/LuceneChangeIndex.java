...
name|commitWithinMs
decl_stmt|;
DECL|method|GerritIndexWriterConfig (Version version, Config cfg, String name)
specifier|private
name|GerritIndexWriterConfig
parameter_list|(
name|Version
...
argument_list|)
argument_list|,
literal|"unknown Lucene version for index schema: %s"
argument_list|,
name|schema
argument_list|)
decl_stmt|;
...
literal|"deprecation"
argument_list|)
DECL|method|getSort (Schema<ChangeData> schema, Predicate<ChangeData> p)
specifier|private
specifier|static
name|Sort
name|getSort
...
parameter_list|)
block|{
comment|// Standard order is descending by sort key, unless reversed due to a
comment|// sortkey_before predicate.
if|if
condition|(
name|SortKeyPredicate
operator|.
