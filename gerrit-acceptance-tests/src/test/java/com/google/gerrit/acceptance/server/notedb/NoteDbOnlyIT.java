...
annotation|@
name|Test
DECL|method|noRetryOnLockFailureWithoutAtomicUpdates ()
specifier|public
name|void
name|noRetryOnLockFailureWithoutAtomicUpdates
parameter_list|()
throws|throws
name|Exception
block|{
...
name|fail
argument_list|(
literal|"expected RestApiException"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
argument_list|)
expr_stmt|;
comment|// updateChange was never called, so no message was ever added.
name|assertThat
argument_list|(
name|getMessages
argument_list|(
...
init|)
block|{
comment|// Op lost the race, so the other writer's commit happened first. Op didn't retry, because the
comment|// ref updates weren't atomic, so it didn't throw LockFailureException on failure.
name|assertThat
argument_list|(
name|commitMessages
argument_list|(
