...
name|Exception
block|{
comment|// Create change (the 1st commit on the change's meta branch).
name|PushOneCommit
operator|.
name|Result
name|result
...
argument_list|()
decl_stmt|;
comment|// Add two comments in patch set 1 (the 2nd commit on the change's meta branch).
name|ReviewInput
name|reviewInput
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// Create patch set 2 (the 3rd commit on the change's meta branch).
name|amendChange
argument_list|(
name|changeId
argument_list|)
expr_stmt|;
comment|// Add 'comment3' in patch set 2 (the 4th commit on the change's meta branch).
name|CommentInput
name|comment3
init|=
name|addComment
...
name|id
decl_stmt|;
comment|// Get the target comment.
name|CommentInfo
name|oldComment
init|=
name|gApi
...
argument_list|()
decl_stmt|;
comment|// Delete the target comment.
name|DeleteCommentInput
name|input
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// Check the comment's message has been replaced in NoteDb.
if|if
condition|(
name|notesMigration
operator|.
...
argument_list|)
decl_stmt|;
comment|// Change comment1's message to the expected message.
name|comment1
operator|.
name|message
operator|=
...
return|;
block|}
DECL|method|getCommits (Change.Id changeId)
specifier|private
name|List
argument_list|<
name|RevCommit
