...
expr_stmt|;
block|}
comment|/**    * There was a bug that allowed a user with Forge Committer Identity access    * right to upload a commit and put *votes on behalf of another user* on it.    * This test checks that this is not possible, but that the votes that are    * specified on push are applied only in the name of the uploader.    *    * This particular bug only occurred when there was more than one label    * defined. Hence the test defines a custom label.    *    * When on upload the committer is forged he is automatically added as    * reviewer to the change. This results in a dummy 0 vote. If there was only    * one label this dummy 0 vote collided with any vote for the same label that    * was specified in the push specification and hence the upload failed, which    * means in this case it was not possible to forge a vote.    */
annotation|@
name|Test
...
name|RestApiException
block|{
comment|// add custom label because the bug only allowed to forge a vote when there
comment|// were at least two labels
name|LabelType
name|Q
init|=
name|category
