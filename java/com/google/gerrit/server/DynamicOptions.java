...
parameter_list|)
function_decl|;
comment|/**      * Returns the class that should be used for looking up exported DynamicBean bindings from      * plugins. Override when a particular REST/SSH endpoint should respect DynamicBeans bound on a      * different endpoint. For example, {@code GetDetail} is just a synonym for a variant of {@code      * GetChange}, and it should respect any DynamicBeans on GetChange. GetChange}. So it should      * return {@code GetChange.class} from this method.      */
...
parameter_list|)
block|{
comment|// Ignore unknown plugin options, so that callers who set a plugin option do not fail if the
...
name|log
argument_list|(
literal|"Unknown plugin option %s is ignored."
argument_list|,
name|name
argument_list|)
expr_stmt|;
