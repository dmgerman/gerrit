...
annotation|@
name|AssistedInject
DECL|method|AccountIndexerImpl ( AccountCache byIdCache, DynamicSet<AccountIndexedListener> indexedListener, StalenessChecker stalenessChecker, @IndexExecutor(BATCH) ListeningExecutorService batchExecutor, @GerritServerConfig Config config, @Assisted AccountIndexCollection indexes)
name|AccountIndexerImpl
parameter_list|(
name|AccountCache
name|byIdCache
...
return|;
block|}
DECL|method|autoReindexIfStale (Account.Id id)
specifier|private
name|void
name|autoReindexIfStale
parameter_list|(
...
condition|)
block|{
comment|// Don't retry indefinitely; if this fails the account will be stale.
annotation|@
...
block|}
block|}
comment|/**    * Asynchronously check if a account is stale, and reindex if it is.    *    *<p>Always run on the batch executor, even if this indexer instance is configured to use a    * different executor.    *    * @param id the ID of the account.    * @return future for reindexing the account; returns true if the account was stale.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"deprecation"
argument_list|)
DECL|method|reindexIfStaleAsync ( Account.Id id)
specifier|private
name|com
operator|.
name|google
