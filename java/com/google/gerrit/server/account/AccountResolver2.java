...
name|Result
block|{
DECL|method|create (String input, ImmutableList<AccountState> list, Searcher<?> searcher)
specifier|static
name|Result
name|create
parameter_list|(
...
return|return
operator|new
name|AutoValue_AccountResolver2_Result
argument_list|(
name|input
argument_list|,
name|list
...
return|;
block|}
DECL|method|empty (String input)
specifier|static
name|Result
name|empty
parameter_list|(
...
literal|0
case|:
comment|// TODO(dborowitz): Include information about whether this takes invisible/inactive users
...
annotation|@
name|Inject
DECL|method|AccountResolver2 ( AccountCache byId, Emails emails, AccountControl.Factory accountControlFactory, Provider<InternalAccountQuery> accountQueryProvider, Realm realm)
name|AccountResolver2
parameter_list|(
name|AccountCache
name|byId
...
return|;
block|}
DECL|method|trySearch ( Searcher<I> searcher, String input, Supplier<Predicate<AccountState>> visibilitySupplier)
specifier|private
specifier|static
parameter_list|<
name|I
...
argument_list|()
return|;
comment|// Input is not in this searcher's format.
block|}
name|Stream
argument_list|<
name|AccountState
...
return|;
block|}
comment|// Otherwise, in the case of an empty list, tell searchImpl to proceed to the next searcher.
comment|// The emptiness check must happen after filtering, so that end users can't distinguish between
comment|// the case where the searcher produced no results and where it only produced inactive/invisible
comment|// results.
return|return
name|Optional
operator|.
