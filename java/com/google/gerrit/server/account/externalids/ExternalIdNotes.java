...
decl_stmt|;
DECL|field|disableCheckForNewDuplicateEmails
specifier|private
name|boolean
name|disableCheckForNewDuplicateEmails
decl_stmt|;
DECL|field|readOnly
specifier|private
name|boolean
...
return|;
block|}
DECL|method|setDisableCheckForNewDuplicateEmails ( boolean disableCheckForNewDuplicateEmails)
specifier|public
name|ExternalIdNotes
...
name|Key
argument_list|>
name|extIdsByEmailBefore
init|=
operator|!
name|disableCheckForNewDuplicateEmails
condition|?
name|getExternalIdsByEmail
argument_list|(
name|rw
argument_list|,
name|noteMap
...
argument_list|)
decl_stmt|;
name|checkForNewDuplicateEmails
argument_list|(
name|extIdsByEmailBefore
argument_list|,
name|extIdsByEmailAfter
...
return|;
block|}
DECL|method|getExternalIdsByEmail (RevWalk rw, NoteMap noteMap)
specifier|private
specifier|static
name|Multimap
argument_list|<
...
return|;
block|}
comment|/**    * Checks that the external ID updates didn't result in new duplicate emails.    *    *<p>Duplicate emails that existed before the external IDs updates are ignored.    *    * @param extIdsByEmailBefore external IDs by email before the external ID updates have been    *     performed    * @param extIdsByEmailAfter external IDs by email after the external ID updates have been    *     performed    * @throws ConfigInvalidException if new duplicate emails are found    */
DECL|method|checkForNewDuplicateEmails ( Multimap<String, ExternalId.Key> extIdsByEmailBefore, Multimap<String, ExternalId.Key> extIdsByEmailAfter)
specifier|private
specifier|static
name|void
name|checkForNewDuplicateEmails
...
argument_list|()
decl_stmt|;
comment|// Find all emails that are assigned to multiple external IDs after the external ID updates have
comment|// been performed.
name|SetMultimap
argument_list|<
name|String
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// External IDs that have this email assigned after the external ID updates have been
comment|// performed.
name|SortedSet
argument_list|<
name|ExternalId
...
argument_list|)
expr_stmt|;
comment|// External IDs that already had this email assigned before the external ID updates had been
comment|// performed.
name|SortedSet
argument_list|<
name|ExternalId
...
condition|)
block|{
comment|// No external ID had this email before. This means the updates try to assign it to multiple
comment|// external IDs now.
name|problems
operator|.
name|add
argument_list|(
...
name|format
argument_list|(
literal|"cannot assign email %s to multiple external IDs: %s"
argument_list|,
name|email
argument_list|,
name|extIdsAfter
...
continue|continue;
block|}
comment|// External IDs that get this email newly assigned.
name|Set
argument_list|<
name|ExternalId
operator|.
...
condition|)
block|{
comment|// Ignore, this inconsistency already existed before the external ID updates had been
comment|// performed.
continue|continue;
block|}
comment|// External IDs that had this email assigned before the external ID updates had been
comment|// performed and that still have this email.
name|Set
argument_list|<
name|ExternalId
operator|.
...
condition|)
block|{
comment|// None of the external IDs that had this email before the external ID updates had been
comment|// performed still has this email.
comment|// This means all external IDs that have this email now have it newly assigned.
name|problems
operator|.
name|add
argument_list|(
...
continue|continue;
block|}
comment|// Some external IDs had this email before and the updates try to assign it to additional
comment|// external IDs.
name|problems
operator|.
name|add
argument_list|(
...
name|format
argument_list|(
literal|"cannot assign email %s to external ID(s) %s,"
operator|+
literal|" it is already assigned to external ID(s) %s"
argument_list|,
name|email
argument_list|,
name|extIdsNew
