...
begin_comment
comment|/**  * Reads/writes account data from/to a user branch in the {@code All-Users} repository.  *  *<p>This is the low-level API for account creation and account updates. Most callers should use  * {@link AccountsUpdate} for creating and updating accounts.  *  *<p>This class can read/write account properties, preferences (general, diff and edit preferences)  * and project watches.  *  *<p>The account properties are stored in an 'account.config' config file. Parsing and updating it  * is implemented in this class.  *  *<p>The 'account.config' file is a git config file that has one 'account' section with the  * properties of the account:  *  *<pre>  *   [account]  *     active = false  *     fullName = John Doe  *     preferredEmail = john.doe@foo.com  *     status = Overloaded with reviews  *</pre>  *  *<p>All keys are optional. This means 'account.config' may not exist on the user branch if no  * properties are set.  *  *<p>Not setting a key and setting a key to an empty string are treated the same way and result in  * a {@code null} value.  *  *<p>If no value for 'active' is specified, by default the account is considered as active.  *  *<p>The commit date of the first commit on the user branch is used as registration date of the  * account. The first commit may be an empty commit (if no properties were set and 'account.config'  * doesn't exist).  *  *<p>The preferences are stored in a 'preferences.config' config file. Parsing and updating it is  * implemented by {@link PreferencesConfig} and this class delegates the handling of preferences to  * {@link PreferencesConfig}.  *  *<p>The project watches are stored in a 'watch.config' config file. Parsing and updating it is  * implemented by {@link WatchConfig} and this class delegates the handling of project watches to  * {@link WatchConfig}.  *  *<p>By default preferences and project watches are lazily parsed on need. Eager parsing can be  * requested by {@link #setEagerParsing(boolean)}.  */
end_comment
