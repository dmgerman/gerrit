...
name|Factory
block|{
comment|/**      * Creates an {@code AccountsUpdate} which uses the identity of the specified user as author for      * all commits related to accounts. The Gerrit server identity will be used as committer.      *      *<p><strong>Note</strong>: Please use this method with care and rather consider to use the      * correct annotation on the provider of an {@code AccountsUpdate} instead.      *      * @param currentUser the user to which modifications should be attributed, or {@code null} if      *     the Gerrit server identity should also be used as author      */
DECL|method|create ( @ullable IdentifiedUser currentUser, ExternalIdNotesLoader externalIdNotesLoader)
name|AccountsUpdate
name|create
parameter_list|(
annotation|@
...
expr_stmt|;
block|}
DECL|method|createPersonIdent ( PersonIdent serverIdent, @Nullable IdentifiedUser user)
specifier|private
specifier|static
name|PersonIdent
name|createPersonIdent
